...
object DeltaSimRankRDD2 {
  ...
  def getOutDegree(vertex:VertexId, g:Graph[Int,Int]) : Long = {
    val adjacent = g.edges.filter(e => e.srcId == vertex)
                     ...
                     ) : RDD[((VertexId,VertexId), Double)] = 
  {
    val newDelta = prevDelta.flatMap(pair => {
        ...
        val b_adj = g.edges.filter(e => e.dstId == b)
                        .map(x=>x.srcId)
        val a_adj = g.edges.filter(e => e.dstId == a)
                        .map(x=>x.srcId)
        ...
        .map(k => (k._1, k._2*decay/
                (getOutDegree(k._1._1, g) + getOutDegree(k._1._2, g))))
                        ...
                        : RDD[((VertexId,VertexId), Double)] = 
  {
    ...
    val kv = g.vertices.flatMap(vertex => {
        ...
        val adj = g.edges.filter(e => e.dstId == vertex._1)
                                .map(x => x.srcId)
        val candidates = adj.cartesian(adj)
    ...
    val newDelta = kv.reduceByKey(_ + _).map(k => {
      (k._1, k._2*decay/(getOutDegree(k._1._1, g) + getOutDegree(k._1._2, g)))
    })
    newDelta
  }
  ...
  def compute(g:Graph[Int,Int], numIterations:Int, 
    ...
    val s0Delta = calculateFirstIter(numNodes, g, g.edges)
    ...
    for (i <- 1 to numIterations) {
      val nextIterDelta = calculateNthIter(numNodes, g, prevDelta)
